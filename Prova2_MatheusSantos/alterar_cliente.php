...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_cliente'], $_POST['nome'], $_POST['email'], $_POST['id_perfil'])) {
    ...
    $sql = "UPDATE cliente SET nome_cliente = :nome, email = :email, id_funcionario_responsavel = :id_perfil WHERE id_cliente = :id_cliente";
    $stmt = $pdo->prepare($sql);
...
<body>
    ...
    <?php if ($cliente): ?>
        ...
        <form action="alterar_cliente.php" method="POST" onsubmit="return validarCliente()">
            ...
            <button type="button" onclick="window.location.href='alterar_cliente.php'">Cancelar</button>
        </form>
    <?php endif; ?>
